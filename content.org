...
#+LaTeX: \begin{result} 
  ...
  has been made available on the [[https://hub.docker.com/repository/docker/alegrand38/simgrid3_3_gtnets][DockerHub]]. It can be recovered using:
  #+begin_src shell :results output :exports both
...
** Expectations
   ...
   \begin{figure}
     ...
     \caption{The README which the first set of experiments.}
     \label{fig:README4}
   \end{figure}
   #+END_EXPORT
...
lightweight as possible and easier to maintain.

We now describe how manually to rerun the simulation. We should first
...
sed -e s/bw/1.000000e+05/g -e s/lt/0.00001/g onelink-p-template.xml > /tmp/onelink-p.xml
...
for model in CM02 GTNets LegrandVelho; do
    ...
    /root/simgrid-3.3/examples/msg/gtnets/gtnets     \
       ...
       --cfg=workstation_model:compound --cfg=cpu_model:Cas01 --cfg=network_model:$model;
done;
#+end_src
...
[C1:slave:(2) 0.010974] [msg_test/INFO] ===> Estimated Bw of FLOW[1] : 91128.086469 ;
  message from S1 to C1  with remaining : 0.000000
#+end_example
We recover exactly the expected values and reported on page~\pageref{expected-output}.
...
original data, we decided to work in the =/tmp= of our machine as follows
...
Y=10.6510810055123~ message is really cool as these are the latency and
bandwidth modifiers obtained through a custom linear regression and
this is very familiar. The original paper reports ~.92~ and ~10.4~ (page
5). The difference comes from the fact that the regression we just run
was done using a smaller set of simulations because I didn't want to
wait. 
